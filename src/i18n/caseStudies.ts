...
const en: CaseStudies = {
    ...
    decisions: [
      {
        ...
        why:
          "The whole premise is that my financial data never leaves my machine. Entries live in IndexedDB through Dexie, so there is no account, no server and nothing to breach — and it still persists across reloads.",
      },
      {
  ...
  disputeAgent: {
    ...
    role: "Solo · design + build",
    context: [
      "Chargebacks are the domain I work in at PicPay. I wanted to see what an LLM adds to it — honestly measured, not assumed — and to close my own Python gap, so I built this in Python on purpose instead of in the Node stack I already know.",
    ...
    outcome: [
      ...
      "Deployed as two services — FastAPI on Render, Next.js on Vercel — with per-IP rate limiting on the analysis endpoint.",
    ],
  },

  mixordia: {
    tagline:
      "A signup portal for an electronic-music events brand — inherited as a half-built React + Flask project, rebuilt on Supabase, and given its TV power-on identity.",
    year: "2026",
    role: "Freelance · took over & rebuilt",
    context: [
      "Mixórdia runs electronic-music events and needed a site to turn visitors into subscribers. Another developer had started it — a React front end, a Flask server and a gallery pulled from Google Drive — and the TV-screen look was already part of the brand.",
      "I took the project over in March 2026 and shipped it: new data layer, signup flow with Brazilian privacy-law consent, and the TV power-on intro and CRT signal effects the site is known for.",
    ],
    decisions: [
      {
      ...
      },
      {
        ...
        why:
          "The public key ships to every browser. Writing only through a `SECURITY DEFINER` function means that key can't insert or update the table directly — the smallest possible write surface.",
      },
      {
        title: "The gallery is a folder convention, not a CMS",
        considered: "A database table and an admin screen for events.",
        why:
          "The client uploads a folder named `YYYYMMDD-city` with a file called `flyer`, and the site builds the event card from it. No admin UI to build, nothing new for the client to learn.",
      },
    ],
    outcome: [
      "Live at [mixordiamusic.com](https://www.mixordiamusic.com), deployed on Vercel.",
      "Signup with consent capture, de-duplicated by CPF — or by email for foreign visitors — and a welcome email sent through Resend.",
      "Portuguese and English, UTM capture and analytics on every signup.",
    ],
  },

  vizinhelp: {
    tagline:
      "My postgraduate thesis: a neighborhood skill-exchange app, taken end to end — Angular front end, Node API, Keycloak auth and CI deploying to AWS.",
    year: "2024",
...
const pt: CaseStudies = {
    ...
    decisions: [
      {
        ...
        why:
          "A premissa inteira é que meus dados financeiros nunca saem da minha máquina. Os lançamentos ficam no IndexedDB via Dexie: não há conta, nem servidor, nem nada para vazar — e os dados continuam lá depois de recarregar.",
      },
      {
  ...
  disputeAgent: {
    ...
    role: "Solo · design + desenvolvimento",
    context: [
      "Chargeback é o domínio em que trabalho no PicPay. Eu queria ver o que um LLM acrescenta a ele — medido com honestidade, não presumido — e fechar minha própria lacuna em Python, então construí em Python de propósito em vez de usar a stack Node que já conheço.",
    ...
    outcome: [
      ...
      "Publicado como dois serviços — FastAPI no Render, Next.js na Vercel — com limite de requisições por IP no endpoint de análise.",
    ],
  },

  mixordia: {
    tagline:
      "Um portal de inscrição para uma marca de eventos de música eletrônica — herdado como um projeto React + Flask pela metade, reconstruído em Supabase e com sua identidade de TV ligando.",
    year: "2026",
    role: "Freelance · assumi e reconstruí",
    context: [
      "A Mixórdia faz eventos de música eletrônica e precisava de um site que transformasse visitantes em inscritos. Outro desenvolvedor tinha começado — front-end em React, um servidor Flask e uma galeria puxada do Google Drive — e o visual de tela de TV já fazia parte da marca.",
      "Assumi o projeto em março de 2026 e o coloquei no ar: nova camada de dados, fluxo de inscrição com consentimento da LGPD e a intro de TV ligando e os efeitos de sinal CRT pelos quais o site é conhecido.",
    ],
    decisions: [
      {
      ...
      },
      {
        ...
        why:
          "A chave pública vai para todo navegador. Escrever apenas por uma função `SECURITY DEFINER` faz com que essa chave não consiga inserir nem alterar a tabela diretamente — a menor superfície de escrita possível.",
      },
      {
        title: "A galeria é uma convenção de pastas, não um CMS",
        considered: "Uma tabela no banco e uma tela de administração de eventos.",
        why:
          "O cliente sobe uma pasta chamada `AAAAMMDD-cidade` com um arquivo chamado `flyer`, e o site monta o card do evento a partir dela. Nenhuma tela de admin para construir, nada novo para o cliente aprender.",
      },
    ],
    outcome: [
      "No ar em [mixordiamusic.com](https://www.mixordiamusic.com), publicado na Vercel.",
      "Inscrição com registro de consentimento, sem duplicatas por CPF — ou por e-mail para estrangeiros — e e-mail de boas-vindas enviado pelo Resend.",
      "Português e inglês, captura de UTM e analytics em cada inscrição.",
    ],
  },

  vizinhelp: {
    tagline:
      "Meu TCC da pós-graduação: um app de troca de conhecimento entre vizinhos, de ponta a ponta — front-end em Angular, API em Node, autenticação com Keycloak e CI publicando na AWS.",
    year: "2024",
